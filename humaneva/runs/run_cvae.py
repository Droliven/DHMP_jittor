...
class RunCVAE():
    def __init__(self, exp_name="base_cvaegcndct", device="cuda:0", num_works=0, is_debug=False, args=None):
        ...
        self.cfg = ConfigCVAE(exp_name=exp_name)
    ...
    def train(self, epoch, draw=False):
        ...
        for i, datas in enumerate(dg):
            ...
            with jt.no_grad():
                ...
                padded_gts_dct = dct_transform_cuda(datas, self.dct_m, dct_n=self.cfg.dct_n)  # b, 48, 10
            ...
            outputs = reverse_dct_cuda(out_dct, self.i_dct_m, self.cfg.t_total)  # b, 48, 125
    ...
    def eval(self, epoch=-1, draw=False):
        ...
        for i, datas in enumerate(dg):
            ...
            z = jt.randn(b*self.cfg.nk, self.cfg.z_dim)
            with jt.no_grad():
                padded_inputs = datas[:, :, list(range(self.cfg.t_his)) + [self.cfg.t_his - 1] * self.cfg.t_pred]
                padded_inputs_dct = dct_transform_cuda(padded_inputs, self.dct_m, dct_n=self.cfg.dct_n)  # b, 48, 10
                padded_inputs_dct = padded_inputs_dct.view(b, -1, 3 * self.cfg.dct_n)  # # b, 16, 3*10
                padded_inputs_dct = jt.misc.repeat_interleave(padded_inputs_dct, repeats=self.cfg.nk, dim=0)
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.cfg.epoch_t1 + 1):
            ...
            if epoch % test_interval == 0:
                self.save(
                        os.path.join(self.cfg.ckpt_dir, "models", '{}_err{:.4f}.pkl'.format(epoch, average_all_loss)),
