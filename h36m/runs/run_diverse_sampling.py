...
from ..datas import MaoweiGSPS_Dynamic_Seq_H36m, draw_multi_seqs_2d, get_dct_matrix, dct_transform_cuda, reverse_dct_cuda
...
class RunDiverseSampling():
    def __init__(self, exp_name="", device="cuda:0", num_works=0, is_debug=False, args=None):
        ...
        self.cfg = ConfigDiverseSampling(exp_name=exp_name)
        ...
        self.scheduler = LambdaLR(self.optimizer, origin_lr=self.cfg.lr_t2, epoch_fix_t1=self.cfg.epoch_fix_t2, epoch_t1=self.cfg.epoch_t2)
    ...
    def train(self, epoch, draw=False):
        ...
        for i, (datas, similars) in enumerate(dg):
            ...
            similars = jt.array(similars)
            eps = jt.randn((b, self.cfg.z_dim))

            repeated_eps = jt.misc.repeat_interleave(eps, repeats=self.cfg.nk, dim=0)
            ...
            self.optimizer.backward(all_loss)
            ...
            if draw:
                if i == draw_i:
                    ...
                    output = outputs[bidx, :self.cfg.seperate_head].reshape(self.cfg.seperate_head, -1, 3,
                                                                            self.cfg.t_total).data  # 50, 16, 3, 125
    ...
    def eval(self, epoch=-1, draw=False):
        ...
        for i, datas in enumerate(dg):
            ...
            with jt.no_grad():
                ...
                all_z = all_z_p1
                all_outs_dct = self.model_t1.inference(condition=jt.misc.repeat_interleave(padded_inputs_dct, repeats=self.cfg.nk, dim=0), z=all_z)  # b*h, 16, 30
                all_outs_dct = all_outs_dct.reshape(b * self.cfg.nk, -1, self.cfg.dct_n)  # b*h, 48, 10
                outputs = reverse_dct_cuda(all_outs_dct, self.i_dct_m, self.cfg.t_total)  # b*h, 48, 125
            ...
            if draw:
                if i == draw_i:
                    ...
                    all_outputs = outputs.data.reshape(self.cfg.nk, -1, 3, self.cfg.t_pred)  # 10, 16, 3, 100
    ...
    def run(self):
        for epoch in range(self.start_epoch, self.cfg.epoch_t2 + 1):
            ...
            if epoch % test_interval == 0:
                self.save(
                    os.path.join(self.cfg.ckpt_dir, "models", '{}_err{:.4f}.pkl'.format(epoch, average_hinges)),
