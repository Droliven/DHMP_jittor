...
from ..datas import h36m_valid_angle_check_cuda
...
def loss_kl_normal(mu, logvar):
    ...
    :return:
    """
    kl = -0.5 * jt.sum(1 + logvar - mu.pow(2) - logvar.exp()) / mu.shape[0]
    return kl
...
def loss_recover_history_t1(pred, gt):
    ...
    :return:
    '''
    loss = jt.mean((pred - gt).pow(2).sum(dim=1)) # 沿着 vc
    return loss
...
def loss_recover_history_t2(pred, gt):
    ...
    :return:
    '''
    loss = jt.mean((pred - gt.unsqueeze(dim=1)).pow(2).sum(dim=2)) # 沿着 vc
    return loss
...
def loss_diversity_hinge(pred, minthreshold):  # # [b, 10, 48, 100]
    ...
    dist = jt.norm(pred[:, :, None, :] - pred[:, None, :, :], p=2, dim=-1)  # b, h, h

    dist = jt.nn.relu(minthreshold - dist)
    loss = dist.reshape(b, -1)[:, idx]  # b, 45
    return loss
...
def loss_diversity_hinge_divide(outputs, seperate_head, minthreshold): # b, 50, 42, 60
    ...
    all_hinges = jt.concat(all_hinges, dim=-1).mean(dim=-1).mean()
    return all_hinges
...
def loss_diversity_hinge_between_two_part(part_a, part_b, minthreshold=25):  # [b, 10, 48, 100]
    ...
    dist = jt.norm(part_a[:, :, None, :] - part_b[:, None, :, :], p=2, dim=-1)  # b, h, h
    dist = jt.nn.relu(minthreshold - dist)
    loss = dist.reshape(b, -1)  # b, 45
    return loss
...
def loss_limb_length_t1(pred, gt, parent_17):
    ...
    gt = jt.concat((jt.zeros_like(gt[:, 0:1, :]), gt), dim=1) # b, 17, 3
    limb_gt = jt.norm(gt[:, 1:, :] - gt[:, parent_17[1:], :], dim=2)[:, :, None] # b, 16, 1

    pred = pred.reshape([b, -1, 3, t]) # b, 16, 3, 125
    pred = jt.concat((jt.zeros_like(pred[:, 0:1, :, :]), pred), dim=1) # b, 17, 3, 125
    limb_pred = jt.norm(pred[:, 1:, :, :] - pred[:, parent_17[1:], :, :], dim=2) # 沿着 3：[b, 17, 125]

    loss_limb = jt.mean((limb_gt - limb_pred).pow(2).sum(dim=1))  # 沿着 v
    return loss_limb
...
def loss_limb_length_t2(pred, gt, parent_17):
    ...
    gt = gt[:, :, 0].reshape([b, -1, 3]) # b, 16, 3
    gt = jt.concat((jt.zeros_like(gt[:, 0:1, :]), gt), dim=1) # b, 17, 3
    limb_gt = jt.norm(gt[:, 1:, :] - gt[:, parent_17[1:], :], dim=2)[:, None, :, None] # b, 1, 16, 1

    pred = pred.reshape([b, h, -1, 3, t]) # b, 50, 16, 3, 125
    pred = jt.concat((jt.zeros_like(pred[:, :, 0:1, :, :]), pred), dim=2) # b, 50, 17, 3, 125
    limb_pred = jt.norm(pred[:, :, 1:, :, :] - pred[:, :, parent_17[1:], :, :], dim=3) # 沿着 3：[b, 50, 16, 125]

    loss_limb = jt.mean((limb_gt - limb_pred).pow(2).sum(dim=2))  # 沿着 v
    return loss_limb
...
def loss_recons_adelike(pred, gt):
    ...
    loss_recon_ade = dist.min(dim=1).mean()
...
def loss_recons_mmadelike(pred, similars):
    ...
    loss_recon_multi = dist.min(dim=1)[mask].mean()
    if jt.isnan(loss_recon_multi):
        loss_recon_multi = jt.zeros(1)
...
def loss_valid_angle_t1(pred, valid_ang, data="h36m"):
    ...
    pred = pred.permute(0, 2, 1).reshape(-1, vc)
    if data == "h36m":
        ang_cos = h36m_valid_angle_check_cuda(pred)

    loss = jt.array(np.array([0]))

    for an in ang_names:
        lower_bound = valid_ang[an][0]
        if lower_bound >= -0.98:
            if jt.misc.any(ang_cos[an] < lower_bound):
                loss += (ang_cos[an][ang_cos[an] < lower_bound] - lower_bound).pow(2).mean()
        upper_bound = valid_ang[an][1]
        if upper_bound <= 0.98:
            if jt.misc.any(ang_cos[an] > upper_bound):
...
def loss_valid_angle_t2(pred, valid_ang, data="h36m"):
    ...
    pred = pred.permute(0, 1, 3, 2).reshape(-1, vc)
    if data == "h36m":
        ang_cos = h36m_valid_angle_check_cuda(pred)
...
def loss_diversity_hinge_v2_likedlow(pred, minthreshold=100, scale=50):  # # [b, 10, 48, 100]
    ...
    pred = pred.reshape(b, h, -1)  # b, 10, 48*100
    dist = jt.norm(pred[:, :, None, :] - pred[:, None, :, :], p=2, dim=-1).pow(2)  # b, h, h
    dist = jt.nn.relu(minthreshold - dist)
...
def loss_diversity_hinge_between_two_part_v2_likedlow(part_a, part_b, minthreshold=100, scale=50):  # # [b, 10, 48, 100]
    ...
    dist = jt.norm(part_a[:, :, None, :] - part_b[:, None, :, :], p=2, dim=-1).pow(2)  # b, h, h
    dist = jt.nn.relu(minthreshold - dist)
    dist = (dist / scale).exp()  # b, 45
    loss = dist.reshape(b, -1)  # b, 45
    return loss
...
def compute_diversity(pred):  # [h, 48, 100], [1, 48, 100], [n, 48, 100]
    '''
    ...
    div = jt.norm(pred[:, None, :] - pred[None, :, :], p=2, dim=2) #
...
def compute_diversity_between_twopart(part_a, part_b):  # [10, 48, 100], [1, 48, 100], [n, 48, 100]
    ...
    div = jt.norm(part_a[:, None, :] - part_b[None, :, :], p=2, dim=2).reshape(-1)
    # div = div.mean()
    return div
...
def compute_ade(pred, gt):  # [10, 48, 100], [1, 48, 100], [n, 48, 100]
    ...
    diff = pred - gt
    dist = jt.norm(diff, dim=1).mean(dim=-1)  # 10
    return dist.min()
...
def compute_mmade(pred, gt, gt_multi):  # [10, 48, 100], [1, 48, 100], [n, 48, 100]
    ...
    if not gt_multi.shape[0] == 0:
        all_results = jt.zeros(1+gt_multi.shape[0])
        all_results[0] = dist
        next_idx = 1
...
def compute_bone_percent_error(gt_pose, motions, parent_17):
    ...
    gt_pose = jt.concat((jt.zeros_like(gt_pose[:, 0:1, :]), gt_pose), dim=1)  # 1, 15, 3
    gt_bone = jt.norm(gt_pose[:, 1:, :] - gt_pose[:, parent_17[1:], :], p=2, dim=2)[..., None]  # b, 14, 1

    motions = jt.concat((jt.zeros_like(motions[:, 0:1, :, :]), motions), dim=1)  # n, 15, 3, 60
    motions_bone = jt.norm(motions[:, 1:, :, :] - motions[:, parent_17[1:], :, :], p=2, dim=2)  # n, 14, 60

    diff = jt.abs(motions_bone - gt_bone) / gt_bone  # b, 15, 60
    avg_bone_error = jt.mean(diff)
    min_bone_error = jt.min(diff.mean(dim=[-1, -2]), dim=0)
    max_bone_error = jt.max(diff.mean(dim=[-1, -2]), dim=0)
...
def compute_angle_error(pred, valid_ang, data="h36m"):
    ...
    pred = pred.permute(0, 2, 1).reshape(-1, vc)  # n*100, 48
    if data == "h36m":
        ang_cos = h36m_valid_angle_check_cuda(pred)
...
if __name__ == '__main__':
    # mu = jt.randn((4, 64))
    # logvar = jt.randn((4, 64))
    #
    # r1 = loss_kl_normal(mu, logvar)

    # gt = jt.randn((4, 48, 100))
    # pred = jt.randn((4, 48, 100))
    # r2 = loss_recons(gt, pred)

    # pred = jt.randn((4, 50, 48, 100))
    # valid_angle_path = os.path.join(r"E:\PythonWorkspace\stochastic_human_motion_prediction\diverse_sampling\dataset", "h36m_valid_angle.p")
    # valid_ang = pickle.load(open(valid_angle_path, "rb"))
    # r3 = loss_valid_angle_t2(pred, valid_ang, data="h36m")

    pred = jt.randn((10, 48, 100))
    gt = jt.randn((1, 48, 100))
    similars = jt.randn((5, 48, 100))
    r3 = compute_mmade(pred, gt, similars)
    pass