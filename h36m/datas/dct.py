...
def dct_transform_cuda(data, dct_m, dct_n):
    '''
    B, 60, 35
    '''
    batch_size, features, seq_len = data.shape
    ...
    out_data = jt.matmul(dct_m[:dct_n, :], data)  # [dct_n, 180077*60]
    out_data = out_data.permute(1, 0).reshape(-1, features, dct_n)  # [b, 60, dct_n]
    return out_data

def reverse_dct_cuda(dct_data, idct_m, seq_len):
    '''
    B, 60, 35
    '''
    batch_size, features, dct_n = dct_data.shape

    dct_data = dct_data.permute(2, 0, 1).reshape(dct_n, -1)  # dct_n, B*60
    out_data = jt.matmul(idct_m[:, :dct_n], dct_data).reshape(seq_len, batch_size, -1).permute(1, 2, 0)
    return out_data