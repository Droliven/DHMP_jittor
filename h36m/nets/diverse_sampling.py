...
from jittor.nn import Sequential, Linear, Tanh, BatchNorm1d

from .gcn_layers import GraphConvBlock, ResGCB


class DiverseSampling(Module):
    def __init__(self, node_n=16, hidden_dim=256, base_dim = 64, z_dim=64, dct_n=10, base_num_p1=40, dropout_rate=0):
    ...
    def execute(self, condition, repeated_eps=None, many_weights=None, multi_modal_head=10):
        '''

        :param condition: condition: [b, 48, 25] / [b, 16, 3*10]
        :param repeated_eps: b*h, 64
        :param many_weights:  b*h, 1, 40
        :param multi_modal_head: 50
        :return:
        '''
        ...
        repeat_many_bases = jt.misc.repeat_interleave(bases, repeats=multi_modal_head, dim=0)  # b*h, 10, 64
        many_bases_blending = jt.matmul(many_weights, repeat_many_bases).squeeze(dim=1).view(-1, self.base_dim)  # b*h, 64

        all_mean = self.mean_p1(many_bases_blending)
        all_logvar = self.logvar_p1(many_bases_blending)

        all_z = jt.exp(0.5 * all_logvar) * repeated_eps + all_mean
...
if __name__ == '__main__':
    ...
    # >>> many bases
    logtics = jt.ones((4*100, 1, 10)) / 10  # b*h, 1, 10
